...
export function TestPlan() {
  ...
  const generateTestPlan = async () => {
    ...
    try {
      ...
      const storiesText = selectedStoriesList
        .map(
          (story) =>
            `- ${story.id}: ${story.title} (Priority: ${story.priority}, Status: ${story.status})\n  Description: ${story.description}`
        )
        .join('\n');
...
Provide only valid JSON without any additional text.`;
      ...
      if (provider === 'azure-openai') {
        ...
        const deploymentName = (config as any).deploymentName || 'gpt-4';
      ...
      const parsedResult = JSON.parse(jsonMatch[0]) as TestPlanResult;
      setTestPlanResult(parsedResult);
    } catch (err) {
